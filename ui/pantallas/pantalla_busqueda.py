...
class PantallaBusqueda(ttk.Frame):
    ...
    def _build(self):
        ...
        self.destino_cb = ttk.Combobox(
            grid, textvariable=self.destino_var, state="readonly"
        )
        self.destino_cb.grid(row=1, column=1, sticky="ew", pady=6)

        ttk.Label(grid, text="Fecha (YYYY-MM-DD)").grid(
            row=2, column=0, sticky="w", pady=6
        )
        ttk.Entry(grid, textvariable=self.fecha_var).grid(
            row=2, column=1, sticky="ew", pady=6
        )

        ttk.Label(grid, text="Criterio").grid(
    ...
    def _buscar(self):
        ...
        try:
            resultado = ServiciosAeroIbero.ejecutar_dijkstra(
                origen, destino, criterio
            )
        except Exception as e:
            messagebox.showerror(
                "Error",
                f"No se pudo calcular ruta: {e}"
            )
            return
    ...
    def _continuar(self):
        if not self.app_state.get("opciones"):
            messagebox.showwarning(
                "Sin resultados",
                "Primero busca una ruta para continuar"
            )
            return
